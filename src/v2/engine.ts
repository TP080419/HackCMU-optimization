...
const MAX_CATCH_UP_STEPS_PER_FRAME = 120
  ...
  dispatch(command: GameCommand) {
    ...
    if (command.type === 'loadState') {
      ...
      if (!this.state.paused) {
        this.accumulator += Math.max(0, command.elapsedMs) * this.state.speed
        let steps = 0
        while (this.accumulator >= SIM_STEP_MS && steps < MAX_CATCH_UP_STEPS_PER_FRAME) {
